...
- **Bundled guest-tools discovery understands the system install layout.** `vmm-core::guest_tools::find_bundled_virtio_win_iso` (and the SPICE / WinFsp equivalents) previously embedded the original author's absolute development-machine path and missed the system data directory entirely. The search now uses an executable-relative `../share/libre-vmm/guest-tools/` lookup (matches the `.deb`/`.rpm` layout), an absolute `/usr/share/libre-vmm/guest-tools/` fallback, and the per-user `~/.local/share/libre-vmm/guest-tools/` directory — in that order — followed by the upstream Debian/Fedora path `/usr/share/virtio-win/` as a last-resort fallback for virtio-win.
- **Removed three hardcoded developer-machine paths from committed files.** `.cargo/config.toml` no longer embeds an absolute `-L` linker flag (the local-libvirt build workaround is handled by `build.sh` via `PKG_CONFIG_PATH`); `vmm-gui::views::vm_settings` LibreUEFI firmware constants now point at `/usr/share/libreuefi/` (the canonical system install location a future `libreuefi` package will use); the Flatpak `README.md` example commands use `/path/to/libre-vmm` rather than an absolute developer path.

### Packaging

- **New `libre-vmm-guest-tools` Debian package.** This `arch:all` companion package installs `virtio-win.iso`, `spice-guest-tools.exe`, and `winfsp.msi` to `/usr/share/libre-vmm/guest-tools/windows/`, which the main package's GUI auto-discovers via the new search-path logic above. `libre-vmm` declares it as a `Recommends:`, so `apt install libre-vmm` will pull it in by default but Linux-only-guest users can `apt install --no-install-recommends libre-vmm` to skip it. The split keeps the main `.deb` at ~11 MB; the guest-tools `.deb` weighs ~159 MB (mostly the 754 MB virtio-win ISO, compressed by `dpkg-deb`).
- **`debian/control` reorganised** to declare both binary packages; **`debian/libre-vmm-guest-tools.install`** added; **`debian/compat`** removed (`debhelper-compat (= 13)` build-dep is the single source of truth now); **`debian/install`** removed (`override_dh_auto_install` in `debian/rules` handles `libre-vmm`'s file layout end-to-end).
